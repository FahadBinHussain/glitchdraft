(function () {
  ...
  chrome.storage.local.get("config", (result) => {
    ...
    const debugToggleButton = document.querySelector(
  ...
  function extractWhatsAppChatId() {
    ...
    const activeChat =
      document.querySelector(
        'div[data-testid="cell-frame-container"][class*="active"]',
  ...
  function createUI() {
    ...
    textarea.dataset.placeholder =
      "Type message or paste image... (Alt+S to save)";
  ...
  const resizeObserver = new ResizeObserver((entries) => {
    for (let entry of entries) {
      if (entry.target === ui.container && !isApplyingRemoteResize) {
        ...
        const isVisible =
          !ui.container.classList.contains("hidden") &&
          ...
          chrome.runtime.sendMessage(
            ...
            (settingsResponse) => {
              ...
              chrome.storage.local.set({
                [localCacheKey]: uiPositions[positionKey],
              });
              lastKnownPositionsHash = JSON.stringify(uiPositions[positionKey]);

              // Save to Firestore
              chrome.runtime.sendMessage(
                { action: "saveSettings", settings: { uiPositions } },
                (response) => {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "toggleUI") {
      ...
      const containerLeft =
        Math.random() * (window.innerWidth - containerWidth);
      const containerTop =
        Math.random() * (window.innerHeight - containerHeight);
      ...
      showNotification(
        ...
        "Your saved messages have been updated from another device.",
        "info",
      );
  ...
  function updateSyncInfo(statusData = null) {
    ...
    const syncStatusSection = document.querySelector(
      ".saved-messages-sync-status",
    );
    ...
    const existingProgressBar = syncStatusSection.querySelector(
      ".sync-progress-container",
    );
    ...
    if (statusData.syncInProgress) {
      ...
      const progressInterval = setInterval(() => {
        ...
        chrome.runtime.sendMessage({ action: "getSyncStatus" }, (response) => {
          if (chrome.runtime.lastError) {
            console.error(
              "[CONTENT] Error checking sync status:",
              chrome.runtime.lastError,
            );
          ...
          if (response && response.success && !response.syncInProgress) {
            ...
            console.log(
              "[CONTENT] Progress check timeout, removing progress bar",
            );
            clearInterval(progressInterval);
            if (progressContainer.parentNode) {
              progressContainer.parentNode.removeChild(progressContainer);
  ...
  async function authenticateWithGoogle() {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        if (syncStatusText) {
          syncStatusText.textContent = response
            ? response.message
  ...
  async function saveMessage() {
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        // Save to Firestore
        chrome.runtime.sendMessage(
          { action: "saveDraft", chatId: chatId, messages: savedMessages },
          (saveResponse) => {
      ...
      (response) => {
        ...
        savedMessages.forEach((message, index) => {
          ...
          copyButton.onclick = () =>
            copyToClipboard(
              message.html,
              "Message copied to clipboard! You can now paste it.",
            );
          ...
          editButton.onclick = () =>
            editMessage(message.timestamp, message.html, messageElement);
  ...
  async function copyImageNatively(imageUrl, callback) {
    try {
      ...
      if (typeof ClipboardItem === "undefined") {
        throw new Error(
          "ClipboardItem API is not available in this context. Cannot copy image.",
        );
      }
      ...
      if (firstImageSrc) {
        (async () => {
          try {
            ...
            const fileName = mimeType.includes("gif")
              ? "saved.gif"
              : "saved.png";
            ...
            if (inputField) {
              ...
              try {
                ...
                copyToClipboard(
                  imageHtml,
                  "Failed to add image directly. Copied as HTML instead.",
                );
              }
            }
          } catch (error) {
  ...
  function insertMessageIntoInputField(html) {
    ...
    const specificSelector =
      'div.xzsf02u.x1a2a7pz.x1n2onr6.x14wi4xw.x1iyjqo2.x1gh3ibb.xisnujt.xeuugli.x1odjw0f.notranslate[contenteditable="true"][role="textbox"][spellcheck="true"][data-lexical-editor="true"]';
        ...
        if (elements.length > 0) {
          ...
          if (elements.length > 1) {
            ...
            for (const el of elements) {
              ...
              const rect = el.getBoundingClientRect();
              if (rect.bottom > maxBottom && rect.width > 50) {
                // Ensure it's not a tiny element
                maxBottom = rect.bottom;
                inputField = el;
              }
            }
    ...
    if (inputField && !isOurUIElement(inputField)) {
      try {
        ...
        setTimeout(() => {
          if (!checkTextInserted(inputField, html)) {
            if (config.debugMode) {
              console.log(
                "Paste simulation failed, trying direct insertion as fallback.",
              );
            }
          ...
          setTimeout(() => {
            if (checkTextInserted(inputField, html)) {
              if (config.debugMode) {
                console.log(
                  "Message insertion successful using selector:",
                  matchedSelector,
                );
              }
      ...
      console.error(
        "Could not find suitable message input field (that is not part of our own UI)",
      );
      alert(
        'Could not find message input field. Please make sure you are in a Messenger chat.\n\nIf this error persists, please set debugMode to true in the script and use the Debug button to find working selectors, or use the "Copy" button instead.',
      );
      return false;
    }
  }
  ...
  function debugInputFields() {
    ...
    console.log(
      "Likely input field (bottom-most contenteditable):",
      bottomElement,
    );
    ...
    if (bottomElement) {
      ...
      console.log("Bottom element attributes:", {
        ...
        "data-lexical-editor": bottomElement.getAttribute(
          "data-lexical-editor",
        ),
      ...
      if (bottomElement.className) {
        accurateSelector +=
          "." + bottomElement.className.trim().replace(/\s+/g, ".");
      }
      [
        "role",
        "contenteditable",
      ...
      alert(
        "Check your browser console for detailed results. The most likely input field has been identified with selector: " +
          accurateSelector,
      );
      ...
      try {
        bottomElement.innerHTML =
          "<p>⚠️ Test message - will be removed in 2 seconds ⚠️</p>";
        setTimeout(() => {
    ...
    alert(
      `Potential input field selectors found:\n${matchingSelectorsList}\n\nCheck browser console for details.`,
    );
  }
  ...
  function toggleDebugMode() {
    ...
    const existingDebugButton = menu.querySelector(
      'button[title="Find input field selectors"]',
    );
    ...
    alert(
      `Debug mode ${config.debugMode ? "enabled" : "disabled"}. ${config.debugMode ? "Additional debug options are now available." : ""}`,
    );
  }
  ...
  function copyToClipboard(html, message) {
    ...
    const notificationMessage =
      message || "Message copied to clipboard! You can now paste it.";
  ...
  // Expose useMessage globally so the native overlay (OverlayController) can
  // trigger it via WebView.evaluateJavascript("window.GlitchDraftUseMessage(html)")
  // This handles text insertion AND image file-drop exactly like the extension does.
  window.GlitchDraftUseMessage = useMessage;
})();
